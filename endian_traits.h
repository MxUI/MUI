 ...
 *
 *      - MUI_FLOAT_LITTLE_ENDIAN - this will **assume** that the host has a little endian representation of floating
 * values
 */
...
// Sanity check
#if defined( MUI_INT_BIG_ENDIAN ) || defined( MUI_INT_LITTLE_ENDIAN ) || defined( MUI_FLOAT_BIG_ENDIAN ) || \
    defined( MUI_FLOAT_LITTLE_ENDIAN )
...
#error \
    "MUI Error [endian_traits.h]: Cannot auto-detect integer endianness of platform - please set at compilation (-DMUI_INT_LITTLE_ENDIAN or -DMUI_INT_BIG_ENDIAN) or set to ignore (-DMUI_IGNORE_ENDIAN)"
#endif
#endif
#endif
...
#warning \
    "MUI Warning [endian_traits.h]: Cannot auto-detect float endianness of platform - please set at compilation (-DMUI_FLOAT_LITTLE_ENDIAN or -DMUI_FLOAT_BIG_ENDIAN) or set to ignore (-DMUI_IGNORE_ENDIAN), integer endianness will be followed by default"
#else
#error \
    "MUI Error [endian_traits.h]: Cannot auto-detect float endianness of platform - please set at compilation (-DMUI_FLOAT_LITTLE_ENDIAN or -DMUI_FLOAT_BIG_ENDIAN) or set to ignore (-DMUI_IGNORE_ENDIAN)"
#endif
#endif
#endif
#endif
...
namespace detail {
    ...
    template <size_t size_bytes> struct uint;
    ...
    // functions.
    template <size_t size_bytes> struct endian_converter {
        union data_t {
            char                            buf[size_bytes];
    ...
    // Specialisations
    template <> inline void endian_converter<2>::htobe() {
        data.val = htobe16( data.val );
    }
    template <> inline void endian_converter<2>::betoh() {
        data.val = be16toh( data.val );
    }
    template <> inline void endian_converter<4>::htobe() {
        data.val = htobe32( data.val );
    }
    template <> inline void endian_converter<4>::betoh() {
        data.val = be32toh( data.val );
    }
    template <> inline void endian_converter<8>::htobe() {
        data.val = htobe64( data.val );
    }
    template <> inline void endian_converter<8>::betoh() {
...
template <typename T, typename enable = void> struct endian_traits;

// Integers
template <typename T> struct endian_traits<T, typename std::enable_if<std::is_integral<T>::value>::type> {
    static constexpr bool convert = ( sizeof( T ) > 1 ) && MUI_CONVERT_INT;
};

// Floats
template <typename T> struct endian_traits<T, typename std::enable_if<std::is_floating_point<T>::value>::type> {
