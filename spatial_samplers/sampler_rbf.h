...
public:
	...
	sampler_rbf(REAL r, std::vector<point_type> &pts, INT basisFunc = 0,
		bool conservative = false, bool polynomial = false,
		bool smoothFunc = false, bool readMatrix = false,
				...
				if (!inputFileCAA) {
					...
					if ((CAArow_ == 0) || (CAAcol_ == 0)) {
						std::cerr << "Error on the size of connectivityAA matrix in matrixSize.dat! number of rows: "
								  ...
								  << ". Please make sure the matrices were generated with the smoothing function switched on"
								  << std::endl;
					}
					else {
			...
			if (!inputFileHMatrix) {
				...
				if ((tempRow != Hrow_) || ((tempPoints / tempRow) != Hcol_)) {
					std::cerr << "tempRow (" << tempRow
							  << ") does NOT equals to Hrow_ (" << Hrow_
							  << "), or" << std::endl << "(tempPoints/tempRow) ("
							  << (tempPoints / tempRow)
							  << ") does NOT equals to Hcol_ (" << Hcol_ << ")"
							  << std::endl;
				}
			}
