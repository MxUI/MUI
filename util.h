...
inline void set_quiet(bool q) { _quiet = q; }

template <typename REAL> inline REAL clamp(REAL x, REAL l, REAL r) {
  return (x < l) ? l : ((x > r) ? r : x);
}

template <typename REAL> inline REAL sgn(REAL x) { return (x > 0) - (x < 0); }

template <uint N> inline double powr(const double x) {
  return powr<2>(powr<N / 2>(x)) * powr<N % 2>(x);
}

template <> inline double powr<2>(const double x) { return x * x; }

template <> inline double powr<1>(const double x) { return x; }
...
template <typename T> inline T frexp10(T arg, int &exp) {
  if (almost_equal(arg, static_cast<T>(0)))
    exp = 0;
  else
    exp = 1 + static_cast<int>(std::floor(std::log10(std::fabs(arg))));
...
template <typename T> inline T frexp10(T arg, long &exp) {
  if (almost_equal(arg, static_cast<T>(0)))
    ...
    exp = 1 + static_cast<long>(std::floor(std::log10(std::fabs(arg))));
  return arg * std::pow(10, -(exp));
}

template <typename T> inline T frexp10(T arg, long long &exp) {
  if (almost_equal(arg, static_cast<T>(0)))
    exp = 0;
  else
    exp = 1 + static_cast<long long>(std::floor(std::log10(std::fabs(arg))));
  return arg * std::pow(10, -(exp));
}
...
template <typename T1, typename T2, typename T3> struct triple {
  ...
  inline bool operator<(const triple &another) const {
    return i < another.i ||
           (i == another.i &&
            (j < another.j || (j == another.j && k < another.k)));
  }
};
